	...
	public class OBDParser_J1850_PWM : OBDParser
	{
		...
		public override OBDResponseList Parse(OBDParameter param, string response)
		{
			...
			else
			{
				...
				group.Add(line0);
				groups.Add(group);
				if (line0.Length < OBDParser_J1850_PWM.HEADER_LENGTH)
				{
					responseList.ErrorDetected = true;
					return responseList;
				}

				string header = line0.Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH);
				int idx = 1;
				...
				while (idx < groups.Count)
				{
					...
					obd_response.Header = header.Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH);
