...
public class OBDParser_J1850_PWM : OBDParser
{
	...
	public override OBDResponseList parse(OBDParameter param, string response)
	{
		...
		else
		{
			...
			if (str1.Length < OBDParser_J1850_PWM.HEADER_LENGTH)
			{
				obdResponseList.ErrorDetected = true;
				return obdResponseList;
			}
			...
			{
				string strB = str1.Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH);
				int index1 = 1;
				if (1 < arrayList1.Count)
				{
					do
					{
						...
						if (str2.Length >= OBDParser_J1850_PWM.HEADER_LENGTH)
						{
							if (str2.Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH).CompareTo(strB) == 0)
							{
							...
							else
							{
								...
								arrayList2.Add((object)arrayList3);
								strB = str2.Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH);
							}
							++index1;
						}
						else
			...
			label_15:
				...
				if (0 < arrayList2.Count)
				{
					do
					{
						...
						response1.Header = str2.Substring(0, OBDParser_J1850_PWM.HEADER_LENGTH);
